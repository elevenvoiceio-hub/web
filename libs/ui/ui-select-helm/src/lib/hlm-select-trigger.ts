...
export const selectTriggerVariants = cva(
	`border-input [&>ng-icon]:text-muted-foreground focus-visible:border-ring focus-visible:ring-ring/50 dark:bg-input dark:hover:bg-input/50 shadow-xs flex w-full items-center justify-between gap-2 whitespace-nowrap rounded-md border bg-input w-i px-3 py-2 px-3 py-2 text-sm outline-none transition-[color,box-shadow] focus-visible:ring-[3px] disabled:cursor-not-allowed disabled:opacity-50 data-[size=default]:min-h-9 data-[size=sm]:h-8 [&>ng-icon]:pointer-events-none [&>ng-icon]:size-4 [&>ng-icon]:shrink-0`,
	{
		variants: {
			error: {
